...
import { Play, Pause, Music, Zap, Layers, Settings, Activity, Disc, Radio, Globe, Info } from 'lucide-react';
...
import { VisualEngine } from './lib/visuals/VisualEngine';
...
export default function App() {
  ...
  const initAudio = () => {
    ...
    const conductor = new Conductor(ctx, state, (step, time) => {
      const pattern = drumEngine.getPattern(state.style, step);

      if (pattern.kick) {
        drumEngine.playKick(time, pattern.kick);
        visualEngineRef.current?.addPulse('#ef4444');
      }
      if (pattern.snare) {
        drumEngine.playSnare(time, pattern.snare);
        visualEngineRef.current?.addPulse('#f59e0b');
      }
      if (pattern.hihat) {
        drumEngine.playHiHat(time, pattern.hihat, step % 8 === 7);
        visualEngineRef.current?.addPulse('#22d3ee');
      }

      // Band members
      bandEngine.playBass(time, state, step);
      bandEngine.playLead(time, state, step);

      // Update XP based on density and chaos
      setXp(prev => prev + Math.floor(state.density * 0.1 + state.chaos * 5));
    });
    ...
    if (canvasRef.current) {
      visualEngineRef.current = new VisualEngine(canvasRef.current, state);
    }
  ...
  const updateState = (updates: Partial<BandState>) => {
    ...
    setState(newState);
    conductorRef.current?.updateState(newState);
    visualEngineRef.current?.updateState(newState);
  };
    ...
    <div className="h-screen w-screen bg-[#050810] text-[#c8d8e8] font-mono overflow-hidden flex flex-col">
      ...
      <main className="flex-1 relative flex">
        ...
        <div className="flex-1 relative">
          ...
          <div className="absolute top-4 left-4 pointer-events-none">
            ...
            <div className="flex gap-2">
              <div className="w-32 h-1 bg-[#1a2540] rounded-full overflow-hidden">
                <motion.div
                  className="h-full bg-[#f43f5e]"
                  animate={{ width: `${state.chaos * 100}%` }}
                />
              </div>
              <div className="w-32 h-1 bg-[#1a2540] rounded-full overflow-hidden">
                <motion.div
                  className="h-full bg-[#22d3ee]"
                  animate={{ width: `${state.density}%` }}
                />
              </div>
            </div>
          </div>
        ...
        <aside className="w-52 bg-[#0a0f1e] border-l border-[#1a2540] p-3 flex flex-col gap-4 z-40 overflow-y-auto">
          <div>
            <div className="text-[9px] text-[#4a5a7a] uppercase tracking-widest mb-2">Q-Table Heatmap</div>
            <div className="w-full h-16 bg-[#050810] border border-[#1a2540] rounded relative overflow-hidden">
              <div className="absolute inset-0 opacity-30">
                {Array.from({ length: 24 }).map((_, i) => (
                  <div
                    key={i}
                    className="absolute w-1 bg-[#22d3ee]"
                    style={{
                      left: `${i * 8}px`,
      ...
      {/* Bottom Controls */}
      <footer className="h-24 bg-[#0a0f1e] border-t border-[#1a2540] flex items-center px-4 gap-6 z-50">
        <ControlSlider
          label="BPM"
          value={state.bpm}
          min={60}
          max={200}
          onChange={v => updateState({ bpm: v })}
        />
        ...
        <ControlSlider
          ...
          max={100}
          unit="%"
          onChange={v => updateState({ swing: v })}
        />
        <div className="w-[1px] h-10 bg-[#1a2540]" />
        <ControlSlider
          label="Chaos"
          value={Math.round(state.chaos * 100)}
          min={0}
          max={100}
          unit="%"
          onChange={v => updateState({ chaos: v / 100 })}
        />
        <div className="w-[1px] h-10 bg-[#1a2540]" />
        <ControlSlider
          label="Density"
          ...
          min={0}
          max={100}
          onChange={v => updateState({ density: v })}
        />
        <div className="w-[1px] h-10 bg-[#1a2540]" />
        <div className="px-4">
          <button
            onClick={togglePlay}
            className="px-8 py-2.5 border-2 border-[#22d3ee] text-[#22d3ee] font-bold tracking-widest rounded hover:bg-[#22d3ee]/10 transition-all flex items-center gap-2"
          >
