...
// Returns a desaturated, lightened version of the style colour — neutral enough
// for the Munker-White assimilation effect but still tinted.
...
export class ThreeVisualEngine {
  ...
  // Three-layer particle system for phase interference
  private layers: ParticleLayer[] = [];

  // Beat reactivity
  private beatPulse = 0; // 0–1, decays per frame

  // Munker-White background
  ...
  constructor(canvas: HTMLCanvasElement, state: BandState) {
    ...
    // ── Munker-White stripe background ──────────────────────────────────────
    this.bgScene  = new THREE.Scene();
    this.bgCamera = new THREE.OrthographicCamera(-1, 1, 1, -1, 0, 1);
    const stripeA = new THREE.Color(STYLE_COLORS[state.style]).multiplyScalar(0.36);
    this.bgMat = new THREE.ShaderMaterial({
      uniforms: {
        colorA:     { value: stripeA },
    ...
    // ── Three particle layers (size × speed ratios create phase moiré) ──────
    ...
    // frequencies creates the interference / moiré secondary phase the user saw.
    ...
    this.membrane = new THREE.Mesh(new THREE.IcosahedronGeometry(2.2, 2), memMat);
  ...
  private animate = () => {
    ...
    // ── Particle layers — each drifts at its own speed (creates phase moiré) ─
    this.layers.forEach(layer => {
      const sm  = layer.speedMult;
      const pos = layer.positions;

      // Scatter jolt proportional to beatPulse (decays naturally next frame)
      ...
      // Size pulses with beat — each layer scales slightly differently (adds to phase feel)
      layer.mat.size = layer.sizeBase * (1 + this.beatPulse * (0.5 - layer.speedMult * 0.1));
    });

    // ── Membrane — scale + rotate, reacts to beat ───────────────────────────
  ...
  // Differentiated pulse: kick scatters, snare flashes, hihat tints
  ...
  public updateState(newState: Partial<BandState>) {
    ...
    this.bgMat.uniforms.colorA.value.copy(new THREE.Color(hex).multiplyScalar(0.36));
  }
